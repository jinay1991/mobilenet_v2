...
class Bottleneck(tf.keras.layers.Layer):
    def __init__(self, in_ch, out_ch, multiplier, repetitions, strides, enable_expansion=True):
        ...
        for i in range(0, self.repetitions):
            ...
            self.depthwise_conv.append(DepthwiseConv2D(input_channels * multiplier, ksize=3, strides=strides if first_layer else 1))
            self.project_conv.append(Conv2D(input_channels * multiplier, output_channels, ksize=1, strides=1, activation=False))
            self.add.append(Add())
            first_layer = False
